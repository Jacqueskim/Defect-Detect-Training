...
###################################################################################################
"""
Tiny SSD (Single Shot Detector) Variant Model for Almond Detection
"""
...
class TinySSDBaseAlmond(nn.Module):
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.fire9 = ai8x.FusedConv2dBNReLU(32, 32, 3, padding=1,
                                                   **kwargs)
    ...
    def forward(self, image):
        ...
        fire9_feats = self.fire9(fire8_feats)  # (N, 32, 56, 42)


        fire10_feats = self.fire10(fire9_feats)  # (N, 32, 28, 21)
...
class AuxiliaryConvolutions(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.conv12_1 = ai8x.FusedConv2dBNReLU(32, 16, 3, padding=1, **kwargs)  # (N, 16, 28, 21)
        self.conv12_2 = ai8x.FusedMaxPoolConv2dBNReLU(16, 16, 3, padding=1,  **kwargs)  # (N, 16, 14, 10)
...
class PredictionConvolutions(nn.Module):
    ...
    def forward(self, fire4_feats, fire9_feats,  conv12_2_feats):
        ...
        # print(f"{l_fire9.shape} the shape of lfire9")
        # print(f"{l_conv12_2.shape} the shape of conv12")
        # print(f"{locs.shape} the locs.shape")
        classes_scores = torch.cat([ c_fire9,  c_conv12_2],
                                   dim=1)
        return (locs, classes_scores)


class TinierSSDAlmond(nn.Module):
    """
    ...
    def __init__(self, num_classes,
        ...
        self.base = TinySSDBaseAlmond(**kwargs)
    ...
    def create_prior_boxes(aspect_ratios=default_aspect_ratios, device='cpu'):
        ...
        fmap_dims = {'fire9': (56,42),
                     'conv12_2': (14,10)}

        fmaps = list(fmap_dims.keys())

        obj_scales = {'fire9': 0.2,
                      'conv12_2': 0.4}
        ...
        if True in (len(aspect_ratios_list) !=
                    len(TinierSSDAlmond.default_aspect_ratios[0])
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        # print(f"the previous predicted scores {predicted_scores[0][0:5]}")
        predicted_scores = F.softmax(predicted_scores, dim=2)
        # print(f"the outcome predicted scores {predicted_scores[0][0:5]}")
        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                # print(f"what shall be the min score {score_above_min_score} and the shape {score_above_min_score.shape}")
...
def ai85tinierssdalmond(pretrained=False, **kwargs):
    """
    Constructs a Tinier SSD model
    """
    assert not pretrained
    return TinierSSDAlmond(aspect_ratios=TinierSSDAlmond.default_aspect_ratios, **kwargs)


models = [
    {
        'name': 'ai85tinierssdalmond',
        'min_input': 1,
        'dim': 2,
    }
]
