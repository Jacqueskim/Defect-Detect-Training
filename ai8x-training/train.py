...
def main():
    ...
    # print("args for the train", args)
    # args = "Namespace(act_mode_8bit=False, activation_stats=[], avg_pool_rounding=False, batch_size=2, cnn='ai85tinierssd', compress=None, cpu=False, csv_prefix=None, data='data', dataset='Almond_100', deterministic=True, device=85, display_confusion=False, display_embedding=False, display_prcurves=True, earlyexit_lossweights=None, earlyexit_thresholds=None, effective_test_size=1.0, effective_train_size=1.0, effective_valid_size=1.0, epochs=100, evaluate=False, extras=None, generate_sample=None, gpus=None, kd_distill_wt=0.5, kd_pretrained=False, kd_resume='', kd_start_epoch=0, kd_student_wt=0.5, kd_teacher=None, kd_teacher_wt=0.0, kd_temp=1.0, kernel_stats=False, load_model_path='', load_serialized=False, log_params_histograms=False, lr=0.001, masks_sparsity=False, momentum=0.9, name=None, nas=False, nas_kd_resume_from='', nas_policy='', nas_stage_transition_list=None, no_tensorboard=True, num_best_scores=1, obj_detection=True, obj_detection_params='parameters/obj_detection_params_svhn.yaml', optimizer='Adam', output_dir='logs', pretrained=False, print_freq=200, qat_policy='policies/qat_policy_svhn.yaml', qe_bits_accum=32, qe_bits_acts=8, qe_bits_wts=8, qe_calibration=None, qe_clip_acts=<ClipMode.NONE: 0>, qe_clip_n_stds=None, qe_config_file=None, qe_convert_pytorch=False, qe_dynamic=False, qe_mode=<LinearQuantMode.SYMMETRIC: 1>, qe_mode_acts=None, qe_mode_wts=None, qe_no_clip_layers=[], qe_no_quant_layers=[], qe_per_channel=False, qe_pytorch_backend='fbgemm', qe_save_fp_weights=False, qe_scale_approx_bits=None, qe_stats_file=None, quantize_eval=False, regression=False, reset_optimizer=False, resumed_checkpoint_path='', seed=None, sensitivity=None, sensitivity_range=[0.0, 0.95, 0.05], shap=0, show_train_accuracy='last_batch', sparsity_perf=False, summary=None, summary_filename='model', tblog=False, thinnify=False, truncate_testset=False, use_bias=True, validation_split=0.0, weight_decay=0.0005, workers=4)"
    # Set hardware device
    ...
    if args.resumed_checkpoint_path:
        ...
        if qat_policy is not None:
            ...
            if checkpoint.get('epoch', None) >= qat_policy['start_epoch']:
                ...
                print(f'this is fused bn layers')
    ...
    for validation_step, (inputs, target) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if args.obj_detection:
                ...
                # print(f"{output_conf} the ouput conf from the model and the shape {output_conf.shape}")
                # print(f"{output_boxes} the ouput_boxes from the model and the shape {output_boxes.shape}")
