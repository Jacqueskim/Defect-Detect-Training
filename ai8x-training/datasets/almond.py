...
sys.path.append('/home/defect/ECE189/ECE189/ai8x-training/data/ALMOND')
sys.path.append('/home/skim945/ECE189/ECE189/ai8x-training/data/ALMOND/processed')
import ai8x
# print(sys.path)
...
class ALMOND(Dataset):
    ...
    def __init__(self, root_dir, d_type, transform=None, img_size=(168, 224), fold_ratio=2,
        ...
        root_dir ='/home/defect/Documents/ECE189/ai8x-training/data'
        ...
        if self.d_type == 'train':
            ...
            # print("this is the train file", self.info_df_csv_file)
    ...
    def __create_info_df_csv(self):
        ...
        if os.path.exists(self.info_df_csv_file):
            ...
            # print("does it print here************************************", flush=True)
            # stdout.flush()
    ...
    def __gen_datasets(self):
        ...
        # print(f"print the df {self.info_df}", flush=True)
        # stdout.flush()
        # print(f"print the df shape {self.info_df.shape}", flush=True)
        # stdout.flush()
        # print(f"the iterations {self.info_df.iterrows()}", flush=True)
        # stdout.flush()
        # printer = [ row for _, row in self.info_df.iterrows()]
        # print(f"the values in the iterrows {printer}", flush=True)
        # stdout.flush()
        for _, row in self.info_df.iterrows():
            num_box = row['num_of_boxes']
            # print(f"this the number of boxes {num_box}")
            # if num_box > 1:
            ...
            # print(f" the current row is {row}", flush=True)
            stdout.flush()
            ...
            # print(img_crp_resized_folded.shape)
...
def ALMOND_get_datasets(data, load_train=True, load_test=True, img_size=(168, 224), fold_ratio=2,
    ...
    if load_train:
        ...
        # print(f"{data_dir} train {train_transform} {img_size} {fold_ratio} {simplified}" ,flush=True)
